    ...
    public confirm(title: string, message: string | React.ComponentType): Promise<boolean> {
        ...
        return new Promise((resolve) => {
            ...
            this.popupPropsSubject.next({
                title: (
                    <span>{title} <i className='work-icon-close' onClick={() => closeAndResolve(false)} /></span>
                ),
                content,
                footer: (
                    <div>
                        <button qe-id='work-popup-ok-button' className='work-button work-button--base' onClick={() => closeAndResolve(true)}>OK</button> <button
                            qe-id='work-popup-cancel-button' className='work-button work-button--base-o' onClick={() => closeAndResolve(false)}>Cancel</button>
                    </div>
                ),
            });
        });
    ...
    public prompt(
        ...
        return new Promise((resolve) => {
            ...
            this.popupPropsSubject.next({
                children: undefined,
                title: (
                    <span>{title} <i className='work-icon-close' onClick={() => closeAndResolve(null)} /></span>
                ),
                ...
                footer: (
                    <div>
                        <button qe-id='prompt-popup-ok-button' className='work-button work-button--base' onClick={(e) => formApi.submitForm(e)}>OK</button> <button
                            qe-id='prompt-popup-cancel-button' className='work-button work-button--base-o' onClick={() => closeAndResolve(null)}>Cancel</button>
                    </div>
                ),
            });
        });
