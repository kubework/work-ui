...
import { Select as WorkSelect, SelectOption, SelectProps } from '../select/select';
    ...
    return (
        <div>
            <FormComponent
                ...
                qeid={props.qeId}
                field={props.field}
                className={classNames({ 'work-field': true, 'work-has-value': !!getNestedField(props.formApi.values, props.field) })} />

            {props.label && <label htmlFor={id} className='work-label-placeholder'>{props.label}</label>}
            {getNestedField(props.formApi.touched, props.field) &&
                (props.formApi.errors[props.field] && <div className='work-form-row__error-msg'>{props.formApi.errors[props.field]}</div>)
            }
        </div>
    );
};
...
export const FormSelect = ReactForm.FormField((props: SelectProps & { fieldApi: ReactForm.FieldApi, className?: string }) => {
    ...
    return (
        <div className={classNames(props.className, 'form-field__select')}>
            <WorkSelect {...rest} value={value} multiSelect={props.multiSelect}
