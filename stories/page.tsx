...
const navItems = [{ path: location.pathname, title: 'Sample', iconClassName: 'work-icon-docs' }];
const breadcrumbs = [{
...
const actionMenu = {
    items: [{
        ...
        title: 'New Item 2',
        iconClassName: 'icon work-icon-deploy',
        action: () => {
            // do nothing
        },
    },
...
storiesOf('Page', module)
    .add('default', withState({ selectedFilter: [] })(({ store }: { store: Store<any> }) => (
        <Router history={history}>
            <Route path={location.pathname}>
                <Layout navItems={navItems}>
                    <Page title='Hello world!' toolbar={{
                        breadcrumbs, actionMenu, filter: {
                            items: [
                                {
        ...
        const manyNavItems = [];
        for (let i = 0; i < 10; i++) {
            manyNavItems.push({ path: location.pathname + '/' + i, title: 'Sample', iconClassName: 'work-icon-docs' });
        }
        return (
            <Router history={history}>
                <Route path={location.pathname}>
